...
export class UserJoinComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._route.params.subscribe((routeData: Params) => this.sessionId = routeData.sid);
    this.createForm();
  }
  ...
  onSubmit(evt: Event) {
    ...
    this._store.dispatch(initSessionStore(sessionState));
    this._router.navigate([`/session/${sessionState.sessionId}/user/${sessionState.username}`]);
  }
